# Production Pipeline {#sec-production-pipeline}
...
Four CLI scripts in the `scripts/` directory automate the full workflow --- STAC queries, temporal compositing, provenance tracking, and population overlay. This chapter walks through the pipeline, shows the actual command-line calls, and showcases the outputs. The methodology behind each stage is covered in depth in the appendices: @sec-querying-gfm, @sec-temporal-compositing, and @sec-population-exposure.
