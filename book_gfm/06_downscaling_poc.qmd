...
One late addition to the risk picture (details in
@sec-alternative-sources): JRC has confirmed GFDS is unmaintained — its
data pipeline still runs and writes new rasters daily, but development
ended around 2015 and a handover to the Dartmouth Flood Observatory group
remains unresolved. Two consequences for this work: archive the GFDS record
while the service lasts, and treat the physics calibration as more than a
convenience, because it is the route that survives the feed. The underlying
AMSR2/GPM brightness temperatures are public, so the whole signal chain can
be rebuilt from source if GDACS goes dark.
