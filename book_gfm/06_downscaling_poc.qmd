...
Reading guide for the scores, with their machine-learning twins since both
vocabularies are in use on the team: POD (probability of detection, identical
to recall) is the share of radar-flooded cells the map caught; FAR (false
alarm ratio, identical to 1 − precision) is the share of the map's flooded
cells the radar contradicts; CSI (critical success index, identical to
intersection-over-union) folds both into one overlap number.
