...
2. **Allocate**: split that share among the small (~83 m) cells inside each
   pixel, marking the most flood-prone cells as flooded first.
...
if POC is None or not (POC / "downscale_maps.npz").exists():
    raise FileNotFoundError(
        ...
        "01-04 and 06 from the repo root first (inputs are cached or range-read)."
    )
...
```{python}
#| fig-cap: "Flooded area per day over Nigeria. Red: fraction from the R0 lookup (free input, SFED-trained). Blue: licensed FloodScan SFED. At FloodScan's peak, the R0 estimate reaches 84% of its area."
...
GFDS anomaly in @sec-alternative-sources looked visibly blobbier: both panels
share the same 10 km grid (SFED is resampled onto it for comparison), but
sharpness of *pattern* is a different thing. The raw anomaly is smooth
because the radiometer's 7 to 16 km footprints smear across grid cells. The
R0 map looks sharp because its sharpness is inherited: each pixel's lookup
table reproduces that pixel's own SFED value range, so a river pixel maps to
large fractions and its dry neighbour to small ones, regardless of how
smooth the input anomaly is. GFDS supplies the day-to-day dynamics; the
spatial crispness here comes from the training target. The independent
calibrations in the next section do not get this free sharpening.
...
pd.DataFrame({"downscaled GFDS (R0)": score(ds_gfds),
...
of one quantile, a tuned minimum-detectable threshold), re-run the
allocation stage on the R1 fraction, train on the 28-year free archive
rather than one season, add terrain to the historical map, and replicate on
a second basin. The free input's finer working resolution (~10 km vs ~22 km)
